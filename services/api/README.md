...
- `pnpm --filter @callies-universe/api smoke` — offline, no key/Chrome: boots on an
  ephemeral port, asserts `/voice` (silent clips) and `/render?dryRun=1` (voice→spec
  wiring). In the root `verify` chain.
- `pnpm --filter @callies-universe/api render-smoke` — **opt-in**, gated on
