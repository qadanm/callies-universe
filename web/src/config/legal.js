// Legal suite — ONE shared source, rendered per-site so each app's wording is
// accurate (Apple checks the privacy policy matches the app). These are genuine
// STARTING DRAFTS, not legal advice: have counsel review + FILE the actual
// trademark/copyright registrations (a webpage asserts rights; filings secure them).

export const ENTITY = "Callie's Universe";
export const CONTACT = "legal@calliesuniverse.com";
export const SUPPORT = "support@calliesuniverse.com";
...
/** Build the legal docs for a given site (subject + media wording interpolated). */
export function getLegal(site) {
  const subj = site?.subjectNoun || "what you submit";
  const media = site?.mediaNoun || "upload"; // "photo" | "screenshot" | "upload"
  const app = site?.name || ENTITY;
  const isApp = site?.kind === "app";
  const submission = isApp ? `your ${media} (the ${subj} you want roasted)` : `what you submit (a photo or screenshot)`;

  return {
    terms: {
      title: "Terms of Service",
      intro: `These Terms govern your use of ${app} and its apps and websites (the “Service”), part of ${ENTITY}. By using the Service you agree to them.`,
      sections: [
        { h: "The Service", body: P(`${ENTITY} makes entertainment: AI-assisted comedic “roasts” of things you submit, delivered as text and short videos performed by original fictional characters. It is for fun and is not advice of any kind.`) },
        { h: "Your content & your license to us", body: P(`You keep ownership of what you upload (“Your Content” — e.g. ${submission}). You grant us a worldwide, royalty-free license to host, process, and transform Your Content solely to generate and deliver your roast and to operate the Service. You confirm you have the right to upload it — don't upload other people's photos, private messages, or copyrighted material without permission.`) },
        ...
        { h: "Acceptable use", body: P(`You must follow our Acceptable Use Policy. The comedy targets the submitted ${subj} — never a real person, group, or protected class — and you agree not to use the Service to harass, defame, or harm anyone.`) },
    ...
    privacy: {
      ...
      intro: `How ${app} handles your data. Short version: we use ${submission} to make your roast, we don't sell your data, and we keep as little as possible.`,
      sections: [
        { h: "What we collect", body: P(`The ${media} you submit (to generate the roast)${subj === "car" ? "; the make/model you enter" : ""}; an anonymous device identifier for credits; and basic, privacy-respecting analytics. No account or email is required to use the core experience.`) },
        { h: "How we use it", body: P(`Your ${media} is sent to our processing service only to understand the ${subj} and generate the roast and video. We aim the comedy at the ${subj}, never at a person. We use AI providers (e.g., Anthropic for text, ElevenLabs for voices) under their terms to produce the Output.`) },
        { h: "Retention", body: P(`We don't sell your data. We retain ${media}s only as long as needed to deliver and (briefly) support your roast, then delete them. Generated Output you save lives on your device unless you share it.`) },
    ...
    "acceptable-use": {
      ...
      sections: [
        ...
        { h: "The comedy", body: P(`Roasts are aimed at the submitted ${subj} — never a person's body, identity, or worth. We may refuse, filter, or remove content and may suspend accounts that break these rules.`) },
    ...
    ip: {
      ...
      sections: [
        { h: "Our marks", body: P(`“${ENTITY},” “${app},” the other “Roast My ___” names and logos, and the character names are trademarks of ${ENTITY}. You may not use them without written permission, or in any way that implies affiliation or endorsement.`) },
