...
| **60–90s gets a distribution bonus IF watch-time holds**; 20–35s wins completion/rewatch | Ship a **tight 25s share cut** (default) + an optional **50–60s "director's cut"** |
...
1. **The Reel** — 9:16 MP4. Default **25s share cut**; optional **50–60s director's cut**.
2. **The Verdict Card** — a still PNG: the savage one-liner + the score. For Stories / feed / screenshot (screenshot-bait travels where video can't).
...
| 4–20s | **The set** | 3–4 beats, escalating; word-karaoke captions; punch-words pop; SFX stingers on punchlines. |
...
*Already wired* (`fetchVoice` / `ELEVENLABS_API_KEY`) — needs the key, per-comic **voice
...
*(Avoid PlayHT — acquired by Meta, deprecating.)*
...
- A ~30s render ≈ **$1–1.5** → **gate behind premium credits**; run as an async job
...
**5. Sound design — taste layer.** Rimshot/airhorn stingers on punchlines, a light crowd
...
trap on a downloadable/generated file — don't.)*
...
- **Duet/stitch bait** — leave a 1–2s reaction beat at the end; "react to your friend's roast."
...
Premium credits = avatar performance (~$1–1.5) + premium voices + the Panel format. This
